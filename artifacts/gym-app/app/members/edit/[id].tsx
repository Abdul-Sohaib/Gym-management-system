...
export default function EditMemberScreen() {
  ...
  const loadMember = useCallback(async () => {
    try {
      const data = await api.get<{ success: boolean; data: any }>(`/members/${id}`);
      const m = data.data;
      setForm({
        fullName: m.fullName,
        email: m.email,
        ...
        totalFeesPaid: m.totalFeesPaid?.toString() || "0",
        notes: m.notes || "",
        profilePhoto: m.profilePhoto || "",
      });
      setStartDate(new Date(m.packageStartDate));
    } catch (err: any) {
  ...
  const pickPhoto = async () => {
    ...
    if (!result.canceled && result.assets[0].base64) {
      update("profilePhoto", `data:image/jpeg;base64,${result.assets[0].base64}`);
    }
  };
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      <View style={[styles.topBar, { paddingTop: topPad + 8, backgroundColor: colors.background, borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.screenTitle, { color: colors.foreground }]}>Edit Member</Text>
        <View style={{ width: 40 }} />
      </View>
      ...
      >
          ...
          <TouchableOpacity onPress={pickPhoto} style={[styles.photoPicker, { backgroundColor: colors.card, borderColor: colors.border }]}>
            {form.profilePhoto ? (
              ...
              <View style={styles.photoPlaceholder}>
                ...
                <Text style={[styles.photoText, { color: colors.mutedForeground }]}>Change Photo</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>
        ...
        <View style={{ paddingHorizontal: 20 }}>
          <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
            ...
            <FormInput label="Full Name" value={form.fullName} onChangeText={(v) => update("fullName", v)} icon="account" error={errors.fullName} />
            <FormInput label="Email" value={form.email} onChangeText={(v) => update("email", v)} keyboardType="email-address" autoCapitalize="none" icon="email" error={errors.email} />
            <FormInput label="Phone" value={form.phone} onChangeText={(v) => update("phone", v)} keyboardType="phone-pad" icon="phone" error={errors.phone} />
          </View>
          ...
          <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
            ...
            <FormInput label="Fees Paid" value={form.totalFeesPaid} onChangeText={(v) => update("totalFeesPaid", v)} keyboardType="decimal-pad" icon="currency-usd" />
          </View>

          <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.cardBorder }]}>
            <Text style={[styles.sectionTitle, { color: colors.primary }]}>Notes</Text>
            <FormInput label="Notes" value={form.notes} onChangeText={(v) => update("notes", v)} multiline numberOfLines={3} icon="note-text" />
          </View>

          <TouchableOpacity
            style={[styles.submitBtn, { backgroundColor: colors.primary, opacity: saving ? 0.8 : 1 }]}
